...
function load(){elements.forEach(link=>{console.log("---------------link-------------");console.log(link);link.addEventListener('click',event=>handleBtnClick(event,link));});window.addEventListener('scroll',calculateScrollPercentage);send({action:'viewed'})}
document.addEventListener('visibilitychange',function logData(){if(document.visibilityState==='hidden'){send({action:'left',listenerType:'visibilitychange'})}});if('onpagehide'in window){console.log("Yes On onpagehide");window.addEventListener('pagehide',()=>{console.log('pagehide!');send({action:'left',listenerType:'pagehide'})});}
if('onbeforeunload'in window){console.log("Yes onbeforeunload");window.addEventListener("beforeunload",(event)=>{console.log("I am the 1st one.");send({action:'left',listenerType:'beforeunload'})});}
load()})();